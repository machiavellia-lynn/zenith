...
async def run_weekly_backfill(client=None, conn=None, days=DAYS_BACK):
    ...
    if total_saved > 0:
        ...
        # Step 1: rebuild summary rows (eod_summary from raw_messages)
        ...
        # Step 2: fill price_close from Yahoo BEFORE computing analytics
        try:
        ...
        # Step 3: compute analytics (price_change_pct, phase, action) — needs price_close
        for d in dates_sorted:
            try:
        ...
        log.info(f"✅ Summaries + prices + analytics rebuilt")
